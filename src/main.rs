...
enum Action {
    ...
    Done { id: Option<u32> },
    #[clap(about="Shows all the list names along with the items")]
    Show {
        #[clap(short, long)]
        all: bool,
    },
...
fn main() -> std::io::Result<()> {
    ...
    match &args.command {
        ...
        Some(Action::Show { all }) => {
            if *all {
                list_all(&file)?;
            } else {
                println!("Please specify the correct option for show.");
            }
        }
        None => {
            list(&file)?;
...
fn list_all(f: &File) -> Result<(), std::io::Error> {
    let mut reader = std::io::BufReader::new(f);
    let mut buf = String::new();
    let mut id = 0;
    while reader.read_line(&mut buf)? > 0 {
        ...
        let text: ColoredString = words[1..].join(" ").red();
...
fn add(mut f: File, text: String) -> Result<(), std::io::Error> {
    writeln!(&mut f, "{} {}", "NOT_DONE", text)?;
    Ok(())
}
